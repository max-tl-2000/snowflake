 ...
 */
/*
dbt run --select customer_specific.maximus.custom_queries.AP_OpenFutureMovein --vars '{"target_schema": "SNOW_9F27B14E_6973_48A5_B746_434828265538"}'
*/

{{ config(alias='AP_OpenFutureMovein') }}
...
WHERE coalesce(per."email", '') <> ''
    ...
    AND p."HasSigned" <> 1
    AND (per."mostRecentCommDate" >= dateadd(DAY, - 14, CURRENT_TIMESTAMP())
    AND per."mostRecentCommDate" <= dateadd(DAY, - 7, CURRENT_TIMESTAMP()))
